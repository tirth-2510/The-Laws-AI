...
{query}"""
            ...
            if(listres.tool_calls):
                ...
                if(res.tool_calls[0]['name']=="judgement"):
                    context = search(query=query,collection=res.tool_calls[0]['name'],list=False,radius=0.6)
                else:
                    context = search(query=query,collection=res.tool_calls[0]['name'],list=False,radius=0.5)
            else:
                if(res.tool_calls[0]['name']=="judgement"):
                    context = search(query=query,collection=res.tool_calls[0]['name'],list=False,radius=0.6)
                else:
            ...
            if(context):
                ...
                response=["Sorry, but I couldn't find any relevant information related to your query. Kindly provide additional details or clarify your request so I may assist you accurately.",initial_token+list_token,res.tool_calls[0]['name']]
                return JSONResponse(content=response,status_code=200)
        else:
